...
## Overview
The repository now provides the foundational scaffold for an enterprise-ready Text-to-Speech (TTS) and Speech-to-Text (STT) platform. It is organized as a monorepo so that backend services, machine learning microservices, infrastructure tooling, and documentation can evolve together in a consistent manner.
...
- **Infra (Docker, Docker Compose, k8s stubs coming later):** Container definitions for every service plus supporting databases and caches. Local developers can run everything via a single compose file.
...
infra/       → Dockerfiles, docker-compose stack, and Kubernetes manifest stubs
...
Use the `.env.example` file inside `backend/` as a reference for local configuration.
...
```
This command launches PostgreSQL, Redis, both ML services, and the backend API. Once the containers are running you can check:
